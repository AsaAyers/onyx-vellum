 ...
 */
export const rolloverPlugin = makePlugin("rollover", function ({ tree, ctx }) {
  const { timezone = "UTC", todayDate } = ctx;
  const todayStr = formatDateStr(todayDate, timezone);
  ...
  visit(tree as Root, "listItem", (node, idx, parent) => {
    ...
    if (typeof idx === "number" && parent) {
      ...
      throw new Error("Expected listItem to have an index in its parent list");
    }
  });
});
